...
# reference: https://github.com/hendrycks/robustness/blob/master/ImageNet-C/create_c/make_cifar_c.py
...
test_dir = "/home/ids/imarouf/RevisitingCIL/data/vtab-cil/vtab/test/"
test_data = dset.ImageFolder(test_dir)
print("Test data size:", len(test_data.imgs))
print("Labels data size:", len(test_data.targets))
input_size = 224
size = int((256 / 224) * input_size)
convert_img = trn.Compose([
    trn.Resize(size, interpolation=3),  # to maintain same ratio w.r.t. 224 images
    trn.CenterCrop(input_size),
...
for method_name in d.keys():
    ...
    for severity in range(5, 6):
        ...
        for img, label in zip(test_data.imgs, test_data.targets):
            labels.append(label)
            folder_label, name = img[0].split("/")[-2], img[0].split("/")[-1]
            img = pil_loader(img[0])
            corrupted_img = corruption(convert_img(img))

            level = "Level"+str(severity)
            main_folder = os.path.join('/home/ids/imarouf/RevisitingCIL/data/vtab-C/test/', d[method_name].__name__, level, folder_label)
            if not os.path.exists(main_folder):
                os.makedirs(main_folder)

            img_name = os.path.join(main_folder, name)
            corrupted_img = Image.fromarray(corrupted_img.astype(np.uint8), "RGB")
            corrupted_img.save(img_name)
        print("Done severity", severity, "for corruption", method_name)
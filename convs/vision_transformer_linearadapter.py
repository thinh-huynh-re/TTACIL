...
class AdapterSVD(nn.Module):
    def __init__(self,
        ...
        self.r = 12 #r
        self.adapt_A1 = nn.Parameter(torch.zeros(self.r, self.down_size))  # U:
        self.adapt_E1 = nn.Parameter(torch.zeros(self.r, 1))  # S: Singular values
        self.adapt_B1 = nn.Parameter(torch.zeros(self.n_embd, self.r))  # V:
        self.adapt_bias = nn.Parameter(torch.zeros(self.n_embd))
    ...
    def init_adapters(self):
        ...
        nn.init.normal_(self.adapt_A1, mean=0.0, std=0.002)
        nn.init.normal_(self.adapt_B1, mean=0.0, std=0.002)
        nn.init.ones_(self.adapt_E1)
        nn.init.zeros_(self.adapt_bias)
    ...
    def forward(self, x, add_residual=True, residual=None):
        ...
        weights = (self.adapt_B1 @ (self.adapt_A1 * self.adapt_E1.abs()))
        up = F.linear(x, weights, bias=self.adapt_bias)
...
class BlockLastAdapter(nn.Module):
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, global_pool=False, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768,
        ...
        myblocks = [Block, Block, Block, Block, Block,
                    Block, Block, Block, Block, Block, Block, Block] #BlockLastAdapter

...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 ...
                 tuning_mode='ssf', activate_ssf=False):
        super().__init__()
        self.config = config
        ...
        self.activate_ssf = activate_ssf
        if tuning_mode == 'ssf' and activate_ssf:
            self.ssf_scale_1, self.ssf_shift_1 = init_ssf_scale_shift(config.dim)
            self.ssf_scale_2, self.ssf_shift_2 = init_ssf_scale_shift(config.dim)

    def reset_adapters(self):
        if self.config.ffn_adapt:
            self.adaptmlp.init_adapters()

    def get_adapters_params(self):
        if self.config.ffn_adapt:
            return self.adaptmlp.get_adapters_params()
        else:
            return []
    ...
    def forward(self, x):
        ...
        if self.config.ffn_adapt and self.config.ffn_option == 'parallel':
            if self.tuning_mode == 'ssf' and self.activate_ssf:
                adapt_x = ssf_ada(self.adaptmlp(x, add_residual=False), self.ssf_scale_1, self.ssf_shift_1)
            else:
        ...
        if self.config.ffn_adapt:
            if self.config.ffn_option == 'sequential':
                if self.tuning_mode == 'ssf' and self.activate_ssf:
                    x = ssf_ada(self.adaptmlp(x), self.ssf_scale_1, self.ssf_shift_1)
                else:
                    x = self.adaptmlp(x)

            elif self.config.ffn_option == 'parallel':
                if self.tuning_mode == 'ssf' and self.activate_ssf:
                    x = x + ssf_ada(adapt_x, self.ssf_scale_1, self.ssf_shift_1)
                else:
                    x = x + adapt_x
            else:
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, global_pool=False, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768,
                 ...
                 act_layer=None, weight_init='', tuning_config=None, tuning_mode='ssf', activate_ssf=False):
        super().__init__()
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                config=tuning_config, tuning_mode=tuning_mode_list[i], activate_ssf=activate_ssf
            )
    ...
    def adapters_params(self):
        params = []
        for idx, blk in enumerate(self.blocks):
            params.append(blk.get_adapters_params())
        return params
...
def vit_base_patch16_224_linearadapter(pretrained=False, **kwargs):
    model = VisionTransformer(patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
                              norm_layer=partial(nn.LayerNorm, eps=1e-6), tuning_mode='ssf', activate_ssf=False, **kwargs)
...
def vit_base_patch16_224_in21k_linearadapter(pretrained=False, **kwargs):
    model = VisionTransformer(patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
                              norm_layer=partial(nn.LayerNorm, eps=1e-6), tuning_mode='ssf', activate_ssf=False, **kwargs)
...
def vit_base_patch16_224_dinov2_linearadapter(pretrained=False, **kwargs):
    model = VisionTransformer(patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
                              norm_layer=partial(nn.LayerNorm, eps=1e-6), tuning_mode='ssf', activate_ssf=False, **kwargs)

    # checkpoint_model = torch.load('./pretrained_models/B_16-i21k-300ep-lr_0.001-aug_medium1-wd_0.1-do_0.0-sd_0.0.npz')
    checkpoint_model = timm.create_model("vit_base_patch14_dinov2", pretrained=True, num_classes=0)
    state_dict = checkpoint_model.state_dict()
    # modify the checkpoint state dict to match the model
    # first, split qkv weight into q, k, v
    for key in list(state_dict.keys()):

...
def vit_base_patch16_224_mocov3(pretrained=False, adapter=False, **kwargs):
    ...
    model_kwargs = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, with_adapter=adapter, **kwargs)
    model = _create_vision_transformer('vit_base_patch16_224_in21k', pretrained=True, **model_kwargs)
    del model.head
...
@register_model
def vit_base_patch16_224_sam(pretrained=True, adapter=False, **kwargs):
    """ ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
    ...
    # ckpt = torch.load('pretrained/sam_ViT-B_16.npz', map_location='cpu')['model']
    # state_dict = model.state_dict()
    # state_dict.update(ckpt)
    # model.load_state_dict(state_dict)
    del model.head
...
@register_model
def vit_base_patch16_clip_224(pretrained=True, **kwargs) -> VisionTransformer:
    ...
    # ckpt = torch.load('pretrained/ViT-B-16_CLIP.pt', map_location='cpu')
    # sd_timm = {k.replace('module.visual.trunk.', ''): v for k, v in ckpt['state_dict'].items() if k.startswith('module.visual.trunk')}
    # state_dict = model.state_dict()
    # state_dict.update(sd_timm)
    # model.load_state_dict(state_dict.state_dict())

    model = _create_vision_transformer('vit_base_patch16_clip_224.openai_ft_in12k_in1k', pretrained=pretrained)
    del model.head
    del model.norm
    model.norm = nn.LayerNorm(768)
...
@register_model
def vit_base_patch14_dinov2(pretrained=True, **kwargs) -> VisionTransformer:
    """ ViT-B/14 for DINOv2
    """
...
@register_model
def vit_base_patch16_224_mocov3_adapter(pretrained=True, adapter= True, **kwargs):
...
@register_model
def vit_base_patch16_224_sam_adapter(pretrained=True, adapter=False, **kwargs):
    """ ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
    ...
    model_kwargs = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, with_adapter=False, **kwargs)
    pre_model = _create_vision_transformer('vit_base_patch16_sam_224', pretrained=pretrained, **model_kwargs)
    state_dict = model.state_dict()
...
@register_model
def vit_base_patch16_clip_224_adapter(pretrained=True, **kwargs) -> VisionTransformer:
    """ ViT-B/16 CLIP image tower
    """
    model = VisionTransformer(patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
                              norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)

    pre_model = _create_vision_transformer('vit_base_patch16_clip_224.openai_ft_in12k_in1k', pretrained=pretrained)
    state_dict = model.state_dict()
    pre_state_dict = pre_model.state_dict()
...
@register_model
def vit_base_patch14_dinov2_adapter(pretrained=True, **kwargs) -> VisionTransformer:
    """ ViT-B/16 CLIP image tower
    """
    model = VisionTransformer(patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
                              norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
...
@register_model
def vit_base_patch16_224_mae_adapter(pretrained=True, **kwargs) -> VisionTransformer:
    """ ViT-B/16 CLIP image tower
    """
    model = VisionTransformer(patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
                              norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
